...
module.exports  = async function(req ,res) {
    let resFunc = {
        "true" : (doc) => {
            if (!doc) {
                let errorMessage = `not found Endpoint/${req.params.id}`;
                return res.status(404).json(handleError["not-found"](errorMessage));
            }
            return res.status(200).json(getDeleteMessage("Endpoint" , req.params.id));
        } , 
        "false" : (doc) => {
            return res.status(errorMessage.code).send(errorMessage);
        }
    }
    let [status , doc] = await deleteEndpoint(req);
...
async function deleteEndpoint (req) {
    return new Promise((resolve)=> {
        ...
        mongodb.endpoint.findOneAndDelete({id : id} , (err , doc)=> {
            if (err) {
                console.log(err);
