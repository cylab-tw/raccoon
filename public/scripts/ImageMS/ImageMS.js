    ...
    function ($scope, $translate, imageMSService, commonService) {
        ...
        $scope.QIDO = async function () {
            ...
            imageMSService.QIDO($scope).then(async function (res) {
                ...
                for (let i = 0; i < $scope.dataList.length; i++) {
                    ...
                    for (
                        let x = 0;
                        x < $scope.dataList[i].oldIdentifier.length;
                        x++
                    ) {
                        if (
                            $scope.dataList[i].oldIdentifier[x].use ==
                            "official"
                        ) {
                            $scope.dataList[i].studyID =
                                $scope.dataList[i].oldIdentifier[
                                    x
                                ].value.substring(8);
                        }
                    }
        ...
        $scope.downloadStudy = function (iItem) {
            ...
            imageMSService.downloadZip(url).then(function (res) {
                ...
                let fileNameMatchs = res
                    .headers("Content-Disposition")
                    .matchAll(/filename=(.*zip)/gi);
        ...
        $scope.tagsStudyClick = function (iItem) {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if ($scope.isObject($scope.tagsStudy[keys[i]])) {
                    $scope.setJsonViewer(
                        "#tagsStudyJsonRenderer_" + keys[i],
                        $scope.tagsStudy[keys[i]]
                    );
                }
            }
        };
        ...
        $scope.tagClick = function (iItem, iTagName) {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                if ($scope.isObject(tagItem[keys[i]])) {
                    $scope.setJsonViewer(
                        `#tags${iTagName}JsonRenderer_` + keys[i],
                        tagItem[keys[i]]
                    );
                }
            }
